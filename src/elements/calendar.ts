export class CalendarElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.dayFnMapper = (day) => {
      ...
      // const detail = this.formatDate(day.dataset)
      const event = new CustomEvent('selected', {detail})
      this.dispatchEvent(event)
    }
  }

  formatDate(value: DayDataSet) {
  ...
  updateSelected(day: DayCell) {
    this.selectableDays.forEach((el) => el.classList.remove('selected'))
    day.classList.add('selected')
  }

  updateDate(year: number, month: number, day: number) {
    this.today = new Date(year, month - 1, day)
    this.showCalendar(month - 1, year)
  }
...
customElements.define('type-calendar', CalendarElement)
